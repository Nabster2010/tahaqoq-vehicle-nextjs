...
import { deleteColor, fetchColors } from "../../app/reducer/colorsSlice";
import Pagination from "../../components/Pagination";
import Spinner from "../../components/Spinner";

const ColorsList = () => {
  const dispatch = useAppDispatch();
  const { loading, error, meta, colors } = useAppSelector(
    (state) => state.colors
  );
  ...
  useEffect(() => {
    dispatch(fetchColors({ page, size }));
  }, [dispatch, page, size]);
                ...
                {colors.map((color) => (
                  ...
                  >
                    ...
                    <td className="whitespace-nowrap py-4 px-6  text-sm font-medium">
                      <Link href={`${router.route}/${color.id}`}>
