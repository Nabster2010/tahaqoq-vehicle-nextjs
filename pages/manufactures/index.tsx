...
import {
  deleteManufacture,
  fetchManufactures,
} from "../../app/reducer/manufacturesSlice";
import Pagination from "../../components/Pagination";
import Spinner from "../../components/Spinner";

const ManufacterList = () => {
  const router = useRouter();
  const dispatch = useAppDispatch();
  const { loading, error, manufactures, meta } = useAppSelector(
  ...
  useEffect(() => {
    dispatch(fetchManufactures({ page, size }));
  }, [dispatch, page, size]);

  if (loading) return <Spinner />;
  if (!manufactures) return <div> no manufacturers found! </div>;
        ...
        <div className="inline-block min-w-full align-middle dark:bg-gray-800">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between">
            ...
            <div className="p-4 pt-0 md:pt-4">
              <button
                type="button"
                onClick={() => router.push("/manufactures/create")}
                ...
                {manufactures.map((manufacter) => (
                  ...
                  >
                    ...
                    <td className="whitespace-nowrap py-4 px-6  text-sm font-medium">
                      <Link href={`${router.route}/${manufacter.id}`}>
                    ...
                    <td className="whitespace-nowrap py-4 px-6  text-sm font-medium">
                      <button
                        ...
                        onClick={() => handleDelete(manufacter.id)}
                      >
                        <svg
                          className="h-6 w-6 "
                          fill="none"
