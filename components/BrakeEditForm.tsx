...
const BrakeEditForm = ({ vehicle }) => {
  const router = useRouter();
  const { id } = router.query;
  const [brake, setBrake] = useState<any>(vehicle.brakeTest);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const { data } = await axios.put(`/api/brake/${brake.id}`, {
        vehicleId: Number(id),
        ...brake,
      });
